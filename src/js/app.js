$(document).ready(function () {
	...
	$('.js-btn').click(function (e) {
		...
		if (typeof DataId == "string") {
			$('#' + DataId).addClass('opened').find('.car').css({
				'opacity': '1'
			});
			var timeoutCar;
	...
	$('.popup').click(function (e) {
		...
		if (e.target == this) {
			$(this).removeClass('opened').removeClass('car_loaded').find('.car').css({
				'opacity': '0'
			});
	...
	$('.popup_close').click(function (e) {
		e.preventDefault();
		$(this).parents('.popup').removeClass('opened').removeClass('car_loaded').find('.car').css({
			'opacity': '0'
		});
		$('body').css('overflow', 'auto');
	});
	...
	$('.hamburger').on('click', function (e) {
		...
		$('.overlay_nav').addClass('visible').parent().css({
			'overflow': 'hidden'
		});
	});
	$('.overlay_nav').on('click', function () {
		$(this).removeClass('visible').parent().css({
			'overflow': 'auto'
		});
	...
	$('.popup_close').on('click', function (e) {
		e.preventDefault();
		$('.overlay_nav').removeClass('visible').parent().css({
			'overflow': 'auto'
		});
		$(sidebar).removeClass('visible');
	});
